...
INSERT INTO user (vorname, nachname, username, password, treuepunkte) VALUES
	('Hans', 'Hansen', 'hans', '{noop}hans', 0),
	('Peter', 'Petersen', 'peter', '{noop}peter', 50),
	('Anne', 'Admin', 'admin', '{noop}admin', 0),
	('Stefanie', 'Service', 'service', '{noop}service', 0);
...
INSERT INTO user_role (userid, roleid) VALUES
	(
		(SELECT u.id FROM user u WHERE u.username='hans'),
		(SELECT r.id FROM role r WHERE r.name='KUNDE')
	),
	(
		(SELECT u.id FROM user u WHERE u.username='peter'),
		(SELECT r.id FROM role r WHERE r.name='KUNDE')
	),
	(
		(SELECT u.id FROM user u WHERE u.username='service'),
...
INSERT INTO ueberweisung (quellkonto, zielIban, betrag, datum, text) VALUES
	(
		(SELECT k.id FROM konto k WHERE k.name='Girokonto' AND k.userid=(SELECT u.id FROM user u WHERE u.username='hans')),
	...
	(
		(SELECT k.id FROM konto k WHERE k.name='Girokonto' AND k.userid=(SELECT u.id FROM user u WHERE u.username='peter')),
...
CREATE TABLE erlaubte_hosts(
	id BIGINT GENERATED BY DEFAULT AS IDENTITY,
	userid BIGINT NOT NULL,
	hostid BIGINT NOT NULL,
	CONSTRAINT fk_erlhosts_userid FOREIGN KEY (userid) REFERENCES user (id),
	CONSTRAINT fk_erlhosts_hostid FOREIGN KEY (hostid) REFERENCES host (id),
	PRIMARY KEY (id),
	UNIQUE KEY unique_userid_hostid (userid,hostid)
);

INSERT INTO erlaubte_hosts (userid, hostid) VALUES
	(
		(SELECT u.id FROM user u WHERE u.username='admin'),
		(SELECT h.id FROM host h WHERE h.name='127.0.0.1')
	),
	(
		(SELECT u.id FROM user u WHERE u.username='admin'),
		(SELECT h.id FROM host h WHERE h.name='localhost')
	),
	(
		(SELECT u.id FROM user u WHERE u.username='admin'),
		(SELECT h.id FROM host h WHERE h.name='0:0:0:0:0:0:0:1')
	)
;